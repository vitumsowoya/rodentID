...
bwa mem -t 12 cytb_db.fa 10_ZA12L_S3_L001_R1_001.fastq.gz 10_ZA12L_S3_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/ZA12L/cytb_14.fa .
makeblastdb -in cytb_14.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k39_5\b" final.contigs.fa > ZA12L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/ZA12L/cytb_14.fa .
cat ZA12L_cytb_extract.fa cytb_14.fa > cytb_15.fa
mafft --adjustdirection --auto cytb_15.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_15.fa
iqtree -s cleaned_15.fa -m MFP -B 1000 -nt AUTO
