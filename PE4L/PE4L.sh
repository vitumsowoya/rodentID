...
bwa mem -t 12 cytb_db.fa PE4L_S6_L001_R1_001.fastq.gz PE4L_S6_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/PE4L/cytb_1.fa .
makeblastdb -in cytb_1.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k141_2\b" final.contigs.fa > PE4L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/PE4L/cytb_1.fa .
cat PE4L_cytb_extract.fa cytb_1.fa > cytb_2.fa
mafft --adjustdirection --auto cytb_2.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_2.fa
iqtree -s cleaned_2.fa -m MFP -B 1000 -nt AUTO
