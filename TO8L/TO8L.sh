...
bwa mem -t 12 cytb_db.fa TO8L_low_conc_S8_L001_R1_001.fastq.gz TO8L_low_conc_S8_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/TO8L/cytb_7.fa .
makeblastdb -in cytb_7.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k39_4\b" final.contigs.fa > TO8L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/TO8L/cytb_7.fa .
cat TO8L_cytb_extract.fa cytb_7.fa > cytb_8.fa
mafft --adjustdirection --auto cytb_8.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_8.fa
iqtree -s cleaned_8.fa -m MFP -B 1000 -nt AUTO
