...
bwa mem -t 12 cytb_db.fa TO4L_S4_L001_R1_001.fastq.gz  TO4L_S4_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/TO4L/cytb_5.fa .
makeblastdb -in cytb_5.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k59_1\b" final.contigs.fa > TO4L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/TO4L/cytb_5.fa .
cat TO4L_cytb_extract.fa cytb_5.fa > cytb_6.fa
mafft --adjustdirection --auto cytb_6.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_6.fa
iqtree -s cleaned_6.fa -m MFP -B 1000 -nt AUTO
