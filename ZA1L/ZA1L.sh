...
bwa mem -t 12 cytb_db.fa 1_ZA1L_S1_L001_R1_001.fastq.gz 1_ZA1L_S1_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/ZA1L/cytb_10.fa .
makeblastdb -in cytb_10.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k59_3\b" final.contigs.fa > ZA1L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/ZA1L/cytb_10.fa .
cat ZA1L_cytb_extract.fa cytb_10.fa > cytb_11.fa
mafft --adjustdirection --auto cytb_11.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_11.fa
iqtree -s cleaned_11.fa -m MFP -B 1000 -nt AUTO
