...
bwa mem -t 12 cytb_db.fa TO3L_S3_L001_R1_001.fastq.gz TO3L_S3_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/TO3L/cytb_4.fa .
makeblastdb -in cytb_4.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k59_1\b" final.contigs.fa > TO3L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/TO3L/cytb_4.fa .
cat TO3L_cytb_extract.fa cytb_4.fa > cytb_5.fa
mafft --adjustdirection --auto cytb_5.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_5.fa
iqtree -s cleaned_5.fa -m MFP -B 1000 -nt AUTO
