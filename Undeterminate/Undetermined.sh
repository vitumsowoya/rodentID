...
bwa mem -t 12 cytb_db.fa Undetermined_S0_L001_R1_001.fastq.gz Undetermined_S0_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/Undeterminate/cytb_15.fa .
makeblastdb -in cytb_15.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k59_3\b" final.contigs.fa > Undetermined_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/Undeterminate/cytb_15.fa .
cat Undetermined_cytb_extract.fa cytb_15.fa > cytb_16.fa
mafft --adjustdirection --auto cytb_16.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_16.fa
iqtree -s cleaned_16.fa -m MFP -B 1000 -nt AUTO
