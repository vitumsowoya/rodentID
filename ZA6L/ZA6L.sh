...
bwa mem -t 12 cytb_db.fa ZA6L_S7_L001_R1_001.fastq.gz ZA6L_S7_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/ZA6L/cytb_11.fa .
makeblastdb -in cytb_11.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k99_2\b" final.contigs.fa > ZA6L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/ZA6L/cytb_11.fa .
cat ZA6L_cytb_extract.fa cytb_11.fa > cytb_12.fa
mafft --adjustdirection --auto cytb_12.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_12.fa
iqtree -s cleaned_12.fa -m MFP -B 1000 -nt AUTO
