...
bwa mem -t 12 cytb_db.fa TO6L_S10_L001_R1_001.fastq.gz TO6L_S10_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/TO6L/cytb_6.fa .
makeblastdb -in cytb_6.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k79_1\b" final.contigs.fa > TO6L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/TO6L/cytb_6.fa .
cat TO6L_cytb_extract.fa cytb_6.fa > cytb_7.fa
mafft --adjustdirection --auto cytb_7.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_7.fa
iqtree -s cleaned_7.fa -m MFP -B 1000 -nt AUTO
