...
bwa mem -t 12 cytb_db.fa PE1L_S9_L001_R1_001.fastq.gz PE1L_S9_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/PE1L/cytb_db.fa .
makeblastdb -in cytb_db.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k99_1\b" final.contigs.fa > PE1L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/PE1L/cytb_db.fa .
cat PE1L_cytb_extract.fa cytb_db.fa > cytb_1.fa
mafft --adjustdirection --auto cytb_1.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_1.fa
iqtree -s cleaned_1.fa -m MFP -B 1000 -nt AUTO
