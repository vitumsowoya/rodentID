...
bwa mem -t 12 cytb_db.fa PE7L_S5_L001_R1_001.fastq.gz PE7L_S5_L001_R2_001.fastq.gz | samtools view -b - | samtools sort -@ 12 -m 512M -o mapped.sorted.bam
...
cd blast
mv ../final.contigs.fa .
cp /home/vmsowoya/mbewa_study/Version_2/PE7L/cytb_2.fa .
makeblastdb -in cytb_2.fa  -dbtype nucl -out cytb_db
...
cd phylogeny
cp ../final.contigs.fa .
seqkit grep -r -p "^k79_0\b" final.contigs.fa > PE7L_cytb_extract.fa #After this, open the new fa file and rename it by replacing contig name with rodent ID
cp /home/vmsowoya/mbewa_study/Version_2/PE7L/cytb_2.fa .
cat PE7L_cytb_extract.fa cytb_2.fa > cytb_3.fa
mafft --adjustdirection --auto cytb_3.fa > aligned.fa
trimal -in aligned.fa -out trimmed.fa
sed 's/^>_R_/>/' trimmed.fa > cleaned_3.fa
iqtree -s cleaned_3.fa -m MFP -B 1000 -nt AUTO
